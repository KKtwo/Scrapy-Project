...
def save_to_mongodb():
    ...
    while True:
        ...
        source,data = rediscli.blpop(['nanspider:items'])
        item = json.loads(data)
        try:
            if item['image_url']:
                # 存入mongodb中
                sheet.insert({'_id':item['t_id'],'imgs_url':item['image_url'],'user_id':item['user_id']})
                print(u"Success--: " ,item)
        except Exception as e:
            print(e)
            print(u"Repeat--: %r" % item)
def save_to_mysql():
